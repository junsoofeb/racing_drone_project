...
for line in lines:
    ...
    if len(line) > 4:
        ...
        acc = line[13:16]
        gyro = line[17:20]
        new_line = vel + pos + acc + gyro
        new_line = '\t'.join(new_line)
        # '순서 : vel_x  vel_y  vel_z  pos_x  pos_y  pos_z  acc_x  acc_y  acc_z  gyro_x  gyro_y  gyro_z'
        f.write(str(time_cnt) + ' ' + new_line + '\n')
    else:
        f.write(str(time_cnt) + ' ' + line[-1])
    time_cnt += 1

print("LOG data 정리 완료!")
